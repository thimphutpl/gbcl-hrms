...
# 		# ----------------------------------------------------------------
# 		# Step 1: Build to_payables (earnings & deductions) + cc_wise_totals
# 		# ----------------------------------------------------------------
# 		cc_wise_totals = {}  # net pay per cost center

# 		for salary_detail in salary_details:
# 			cc = salary_detail.cost_center or company_cc

# 			# --- earnings/deductions lines (unchanged from your original) ---
# 			posting.setdefault("to_payables", []).append({
# 				"account"        : salary_detail.gl_head,
# 				"credit_in_account_currency" if salary_detail.parentfield == "deductions" else "debit_in_account_currency": flt(salary_detail.amount),
# 				"against_account": default_payable_account,
...
# 			# --- build cc_wise net pay ---
# 			# remittable deductions are paid via remittance entry, exclude from net pay
# 			if salary_detail.parentfield == "earnings":
# 				cc_wise_totals[cc] = cc_wise_totals.get(cc, 0.0) + flt(salary_detail.amount)
# 			elif salary_detail.parentfield == "deductions":
# 				if not salary_detail.is_remittable:
# 					cc_wise_totals[cc] = cc_wise_totals.get(cc, 0.0) - flt(salary_detail.amount)

# 			# --- remittance entries (unchanged from your original) ---
# 			if salary_detail.is_remittable and salary_detail.parentfield == "deductions":
# 				remittance_amount = 0.0
# 				remittance_gl_list = [salary_detail.gl_head, default_employer_pf_account] \
# 					if salary_detail.salary_component == salary_component_pf else [salary_detail.gl_head]
...
# 		# ----------------------------------------------------------------
# 		# Step 2: to_payables net pay credit — one line per cost center
# 		# ----------------------------------------------------------------
# 		for cc, amount in cc_wise_totals.items():
# 			if amount == 0:
# 				continue
# 			posting.setdefault("to_payables", []).append({
# 				"account"                    : default_payable_account,
# 				"credit_in_account_currency" if amount > 0 else "debit_in_account_currency": abs(flt(amount)),
# 				"cost_center"                : cc,
# 				"party_check"                : 0,
# 				"reference_type"             : self.doctype,
# 				"reference_name"             : self.name,
...
# 		# ----------------------------------------------------------------
# 		# Step 3: to_bank — one Dr (payable) + one Cr (bank) per cost center
# 		# ----------------------------------------------------------------
...
# 				posting.setdefault("to_bank", []).append({
# 					"account"                   : default_payable_account,
# 					"debit_in_account_currency" if amount > 0 else "credit_in_account_currency": abs(flt(amount)),
# 					"cost_center"               : cc,
# 					"party_check"               : 0,
# 					"reference_type"            : self.doctype,
# 					"reference_name"            : self.name,
...
# 		# ----------------------------------------------------------------
# 		# Step 4: Create Journal Entries (unchanged from your original)
# 		# ----------------------------------------------------------------
